...
const AuthPage = ({ mode }) => {
  ...
  return (
    <div className="min-h-screen bg-white flex text-zinc-900 font-sans select-none">
      {/* Left Panel - Branding */}
      <LoginLeft />

      {/* Right Panel - Form (Forces white background and light styles) */}
      <div className="flex-1 flex items-center justify-center p-6 sm:p-8 bg-white text-zinc-900">
        <div className="w-full max-w-sm">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  required
                  className="w-full pl-2 py-2 border-b border-zinc-200 focus:outline-none focus:border-zinc-950 text-sm text-zinc-900 bg-white placeholder-zinc-300 pr-8"
                  placeholder="••••••••"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-zinc-300 hover:text-zinc-650 flex items-center justify-center cursor-pointer transition-colors"
                >
                  {showPassword ? (
                    <EyeOffIcon size={14} />
                  ) : (
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 bg-gradient-to-br from-red-600 to-amber-600 text-white font-semibold hover:scale-102 disabled:opacity-40 flex items-center justify-center cursor-pointer mt-2 rounded-lg transition-all"
            >
              {loading && (

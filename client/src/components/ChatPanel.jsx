  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <form onSubmit={handleSubmit} className="p-3 border-t border-zinc-100 bg-white">
        <div className="flex items-center gap-2 border border-zinc-200 rounded-lg px-3 py-2 focus-within:border-zinc-400 transition">
          <input
            type="text"
            value={input}
            onChange={(e) => setInput(e.target.value)}
            placeholder="Ask AI to modify..."
            className="flex-1 text-xs outline-none bg-transparent text-zinc-800 placeholder:text-zinc-400"
            disabled={loading}
          />
          <button
            type="submit"
            disabled={!input.trim() || loading}
            className="p-1 rounded-md bg-zinc-900 text-white disabled:opacity-40 disabled:cursor-not-allowed hover:bg-zinc-800 transition cursor-pointer"
          >
            <SendIcon size={12} />
          </button>
        </div>
